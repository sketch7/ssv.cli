...
conditional `infer` types to *extract* a shape from an existing generic instead of redeclaring it:
...
export type ExtractEntity<T> = T extends EntityMeta<infer E>
	? E
	: T extends EntityDataSourceQuery<infer U>
		? U : never;
```
